...
export default function JobCard({ job, index, filterSkills = [] }) {
  ...
  // Combine matched skills + filter skills to show which matched
      ...
      <div className="job-card-top">
        ...
        <div className="job-company-logo">
          {job.company_logo ? (
            ...
            <div
              className="job-logo-initial"
              style={{ background: getInitialColor((job.company || 'J')[0]) }}
            >
      ...
      {allSkills.length > 0 && (
        <div className="job-skills">
          {allSkills.slice(0, 8).map((skill) => (
            <span
              key={skill}
              className={`job-skill-chip ${matchedSet.has(skill.toLowerCase()) ? 'matched' : 'unmatched'}`}
            >
              {skill}
            </span>
          ))}
          {allSkills.length > 8 && (
            <span className="job-skill-chip unmatched">+{allSkills.length - 8}</span>
          )}
        </div>
      )}
